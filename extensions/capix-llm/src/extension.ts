...
import { DeploysTreeProvider } from "./treeViews";
import { InstancesTreeProvider } from "./cloudPanels";
import { CloudHubProvider } from "./cloudHub";
...
import { CapixAgentHubProvider } from "./agentHub";
...
let cloudHubProvider: CloudHubProvider;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  // The built-in Capix Code surface is authenticated by the IDE's own native
  // OAuth grant. Do not automatically execute the standalone CLI as a token
  // fallback: on macOS that can repeatedly invoke secure-storage consent while
  // the user is merely viewing the IDE, leaving the panel apparently loading.
  // Standalone Capix Code keeps its own explicit `capix-code login` lifecycle.
  authBroker = new CapixAuthBrokerService(context, CapixClient.PRODUCTION_BASE_URL);
  client.setTokenProvider({
    getAccessToken: () => authBroker.getAccessToken(),
  });
  ...
  cloudHubProvider = new CloudHubProvider(client, context.extensionUri);
  ...
  const agentHubProvider = new CapixAgentHubProvider(
    {
  ...
  onboarding = new OnboardingFlow(client, context.extensionUri, {
    ...
    focusCloud: () => {
      void vscode.commands
        ...
        .then(undefined, () =>
          vscode.commands.executeCommand("workbench.view.extension.capix-cloud"),
        );
    },
  });
  ...
  context.subscriptions.push(
    ...
    // Refresh commands
    vscode.commands.registerCommand("capix.refreshDeploys", () => { deploysProvider.load(); void cloudHubProvider.refresh(); }),
    vscode.commands.registerCommand("capix.refreshCatalog", () => { void cloudHubProvider.refresh(); }),
    vscode.commands.registerCommand("capix.refreshInstances", () => { instancesProvider.load(); void cloudHubProvider.refresh(); }),
    vscode.commands.registerCommand("capix.refreshAgents", () => { void cloudHubProvider.refresh(); }),
    vscode.commands.registerCommand("capix.refreshJobs", () => { void cloudHubProvider.refresh(); }),
    vscode.commands.registerCommand("capix.refreshApiKeys", () => { void cloudHubProvider.refresh(); }),
    vscode.commands.registerCommand("capix.refreshProfile", () => { void cloudHubProvider.refresh(); }),
...
async function cmdOpenTerminal(item?: unknown) {
  ...
  const pick = await vscode.window.showQuickPick(
    ...
    }).filter((p) => p.deploymentId.startsWith("dep_")),
...
async function getOrRecoverSshCredential(deploymentId: string): Promise<{ host: string; port: number; privateKey: string; filename: string } | null> {
  try {
    return await client.getStoredSshCredential(deploymentId);
  } catch (error) {
    if (!(error instanceof CapixApiError) || error.status !== 410) throw error;
    const rotate = await vscode.window.showWarningMessage(
      "This instance's previous SSH key is no longer retrievable. Rotate it and revoke the old key?",
      { modal: true, detail: "Capix will install a new public key on the running instance before releasing its matching private key." },
      "Rotate SSH key",
    );
    if (rotate !== "Rotate SSH key") return null;
    await client.rotateSshCredential(deploymentId);
    return client.getStoredSshCredential(deploymentId);
  }
}

async function cmdDownloadSshKey(item?: unknown): Promise<void> {
  const directId = (item as { _instanceId?: string })?._instanceId;
  let deploymentId = directId;
  let label = (item as { label?: string })?.label || "Capix instance";
  if (!deploymentId) {
    await instancesProvider.load();
    const pick = await vscode.window.showQuickPick(
      instancesProvider.instances
        .filter((instance) => instance.id.startsWith("dep_"))
        .map((instance) => ({ label: instance.tier, description: instance.status, deploymentId: instance.id })),
      { placeHolder: "Select an instance whose SSH key you want to save" },
    );
    if (!pick) return;
  ...
  try {
    ...
    if (destination.scheme === "file") await chmod(destination.fsPath, 0o600);
