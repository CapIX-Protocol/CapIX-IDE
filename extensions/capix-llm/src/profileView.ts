...
export class ProfileViewProvider implements vscode.WebviewViewProvider {
  ...
  async refresh() {
    ...
    try {
      ...
      if (billingRes.ok) {
        const br = billingRes as { ok?: boolean; balance?: { usd: string; sol: string; usdc: string }; activeInstances?: number; totalSpent?: string; updatedAt?: string; transactions?: Array<Record<string, unknown>>; instances?: unknown[] };
        this.billing = {
          balance: br.balance!,
