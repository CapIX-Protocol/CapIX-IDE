...
function makeClient(text: string) {
  return {
    streamAgentChat: vi.fn(
      async (_input: unknown, _signal: AbortSignal, onEvent: (e: Record<string, unknown>) => Promise<void>) => {
...
describe("CapixInlineCompletionProvider", () => {
  it("returns undefined when disabled", async () => {
    ...
    const items = await provider.provideInlineCompletionItems(doc as never, doc.__position as never, {} as never, makeToken() as never);
  ...
  it("honors the configured preferred model", async () => {
    ...
    await provider.provideInlineCompletionItems(doc as never, doc.__position as never, {} as never, makeToken() as never);
  ...
  it("serves repeat contexts from cache without a second request", async () => {
    ...
    const first = await provider.provideInlineCompletionItems(doc as never, doc.__position as never, {} as never, makeToken() as never);
    const second = await provider.provideInlineCompletionItems(doc as never, doc.__position as never, {} as never, makeToken() as never);
  ...
  it("returns undefined when cancelled during the debounce window", async () => {
    ...
    const pending = provider.provideInlineCompletionItems(doc as never, doc.__position as never, {} as never, token as never);
    token.cancel();
  ...
  it("aborts the in-flight stream when the token cancels mid-request", async () => {
    ...
    const client = {
      streamAgentChat: vi.fn(async (_input: unknown, signal: AbortSignal, onEvent: (e: Record<string, unknown>) => Promise<void>) => {
        seenSignal = signal;
  ...
  it("toggle flips state, persists config, and updates the context key", async () => {
    ...
    expect(mockExecuteCommand).toHaveBeenCalledWith("setContext", "capix.inlineCompletion.enabled", false);
...
describe("registerInlineCompletions", () => {
  it("registers the provider, commands, edit tracker, and context key", () => {
    ...
    expect(mockExecuteCommand).toHaveBeenCalledWith("setContext", "capix.inlineCompletion.enabled", true);
