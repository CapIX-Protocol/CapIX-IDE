    ...
    async refresh() {
        ...
        try {
            ...
            const [billingRes, treasuryRes, settlementRes, cpxRes] = await Promise.all([
                ...
                this.client.getSettlementStatus().catch(() => ({ ok: false })),
                this.client.getCpxBilling().catch(() => ({ ok: false })),
            ]);
            if (billingRes.ok) {
                const br = billingRes;
                this.billing = {
            ...
            if (settlementRes.ok) {
                const { ok, error, ...status } = settlementRes;
                void ok;
                void error;
...
exports.ProfileViewProvider = ProfileViewProvider;
